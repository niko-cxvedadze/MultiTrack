...
// Public routes
app.route("/auth", auth);
app.route("/admin", adminAuthRoutes); // /admin/login (public) + /admin/me (has own adminAuth)

// Protected admin routes — single middleware for all
const adminProtected = new Hono<AppEnv>();
adminProtected.use("*", adminAuth);
adminProtected.route("/users", users);
adminProtected.route("/upload", upload);
